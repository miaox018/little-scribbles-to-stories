-- Migration: Implement Credit-Based System (Fixed)
-- Date: 2025-09-14
...
BEGIN
  -- Check if user is admin (handle case where is_admin function might not exist)
  BEGIN
    SELECT public.is_admin(user_id_param) INTO is_admin_user;
    IF is_admin_user THEN
      RETURN TRUE;
    END IF;
  EXCEPTION
    WHEN undefined_function THEN
      -- is_admin function doesn't exist, continue with normal credit check
      NULL;
  END;
...
BEGIN
  ...
  EXCEPTION
    WHEN undefined_function THEN
      -- is_admin function doesn't exist, continue with normal credit consumption
      NULL;
  END;
...
DROP FUNCTION IF EXISTS public.can_create_story(UUID);
DROP FUNCTION IF EXISTS public.can_upload_pages(UUID, UUID, INTEGER);
DROP FUNCTION IF EXISTS public.can_regenerate_pages(UUID, UUID, INTEGER);
